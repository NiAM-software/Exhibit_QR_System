...
const getExhibitForUser = asyncHandler(async (req, res) => {
  ...
  try {
    const query = "SELECT exhibit_desc FROM exhibits WHERE exhibit_id=? and active_ind='Y'";
    const [results, fields] = await db.promise().query(query, [id]);

    if (results && results.length > 0) {
      const attachments = await getAttachmentsUtils(id);
      const objectKeys = attachments.map((attachment) => ({
      ...
      res.status(200).json({
        exhibitDescription: results[0].exhibit_desc,
